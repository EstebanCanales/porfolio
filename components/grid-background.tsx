...
type GridBackgroundVariant = "square" | "circle" | "triangle" | "diamond";
...
type GridBackgroundProps = {
  ...
  edgeFade?: number;
  noiseAmount?: number;
  /** Cap the render loop. Default 30fps — halves GPU load vs 60fps. */
  maxFps?: number;
...
const SHAPE_MAP: Record<GridBackgroundVariant, number> = {
  square: 0,
  circle: 1,
  triangle: 2,
  diamond: 3,
...
const GridBackground: React.FC<GridBackgroundProps> = ({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (!wasVisible && entry.isIntersecting && resumeRef.current) {
          resumeRef.current();
  ...
  // Keep speed ref in sync without triggering reinit
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      // Skip render if not enough time has passed — FPS cap
  ...
  // Update uniforms directly without destroying the WebGL context
  useEffect(() => {
    const t = threeRef.current;
    if (!t) return;
